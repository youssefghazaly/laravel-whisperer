<div class=" p-4 mr-10 border-b border-b-grey-400">
  <div class="flex">

    <div class="mr-2 flex-shrink-0">
      <a href="{{route('profile',$tweet->user)}}">
        <img class="rounded-full w-10 mr-2" src="{{asset($tweet->user->avatar )}}">
    </div>

    <div>
      <h5 class="font-bold nb-4">{{ $tweet->user->name}}</h5>
      </a>
      <p class="text-sm">
        {{$tweet->body}}
      </p>
  ...
  <div class="">
    <div class="flex mt-2">
      ...
      <div class="flex {{$tweet->islikeBy(auth()->user())?'text-red-500':'text-red-300'}}">
        <form action="home/{{$tweet->id}}/like" method="post">
          @csrf
          ...
          <button type="submit" class="mt-1 mr-2">{{$tweet->likes?:0}}</button>
        </form>
      </div>

      <div class="flex ml-1 {{$tweet->isDislikeBy(auth()->user())?'text-red-500':'text-red-300'}}">
        <form action="home/{{$tweet->id}}/like" method="post">
          @csrf
          @method('DELETE')
          <svg viewBox="0 0 20 20" class="mt-1  w-4 mr-1">
            <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
              <g class="fill-current">
